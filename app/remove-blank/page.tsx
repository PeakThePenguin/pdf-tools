...
import { loadPdfJsDocument, isPageBlank } from '@/app/lib/pdfjs'
...
export default function RemoveBlankPage() {
  ...
  useEffect(() => {
    if (!file || thumbs.length === 0 || loading) return
    let cancelled = false
    ;(async () => {
      setScanning(true)
      setScanned(false)
      try {
        ...
        for (let i = 1; i <= doc.numPages; i++) {
          if (cancelled) return
          if (await isPageBlank(doc, i)) blanks.add(i - 1)
        }
        doc.destroy()
        if (!cancelled) {
          setSelected(blanks)
        ...
        if (!cancelled) setError((e as Error).message || 'Failed to scan for blank pages.')
      } finally {
        if (!cancelled) setScanning(false)
      }
    })()
    return () => { cancelled = true }
  }, [file, thumbs.length, loading])
  ...
  async function handleRemove() {
    ...
    try {
      ...
      downloadBytes(out, `${stripExt(file.name)}_no_blanks.pdf`)
    } catch (e) {
      setError((e as Error).message || 'Failed to remove blank pages.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <ToolShell title="REMOVE BLANK PAGES" subtitle="Auto-detect and strip out empty pages">
      <div className="px-3 pt-3 space-y-3">
        {!file && <FileDrop accept="application/pdf" onFiles={(f) => { setFile(f[0]); setSelected(new Set()); setScanned(false) }} />}

        {file && (
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 truncate">{file.name}</span>
            <button className="text-xs font-semibold" style={{ color: 'var(--purple)' }} onClick={() => { setFile(null); setSelected(new Set()); setScanned(false) }}>
              Change file
            </button>
          </div>
        )}
        ...
        {thumbs.length > 0 && (
          <>
            ...
            >
              {busy ? 'Removing…' : `Remove ${selected.size || ''} blank page${selected.size === 1 ? '' : 's'}`}
            </button>
          </>
        )}
      </div>
