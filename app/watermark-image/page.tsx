...
export default function WatermarkImagePage() {
  ...
  return (
    <ToolShell title="WATERMARK IMAGE" subtitle="Diagonal text watermark, bottom-left to top-right">
      <div className="px-3 pt-3 space-y-3">
        {!file && <FileDrop accept="image/jpeg,image/png,image/gif" onFiles={(f) => setFile(f[0])} />}

        {file && (
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 truncate">{file.name}</span>
            <button className="text-xs font-semibold" style={{ color: 'var(--purple)' }} onClick={() => { setFile(null); setNote(null) }}>
              Change file
            </button>
          </div>
        )}
